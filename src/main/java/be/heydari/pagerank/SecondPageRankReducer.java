 ...
 * http://heydari.be
 */
public class SecondPageRankReducer extends MapReduceBase implements Reducer<Text, Text, Text, Text> {
			...
			if (matcher.matches()) {
				...
				if ((matcher.group(2)).equals("out")) {            //if this is outgoing html of key page
					...
					if (tempNoOfOut != 0) {
						...
						pageRank = pageRank + (Double.parseDouble(matcher.group(9))) / Commons.TOTAL_PAGES;
					}
				}
			}
		}
		pageRank = 0.85 * pageRank + (1 - 0.85) / Commons.TOTAL_PAGES;
		if (Math.pow((pageRank - oldPageRank), 2) > Math.pow(Commons.PAGE_RANK_THRESHOLD, 2)) {    //there is one computed pagerank not satisfy requirement
